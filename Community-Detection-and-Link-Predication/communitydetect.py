    ...
    while queue:
        ...
        for neighbor in graph.neighbors(parent):
            if neighbor in visited and (node2distances[neighbor] == node2distances[parent]+1):
                    ...
                    """node2parents[n].append(vertex)
                node2num_paths[neighbor]=len(node2parents[n])"""
                    node2num_paths[neighbor] += 1
            elif neighbor not in visited and (node2distances[parent] < max_depth):
                    queue.append(neighbor)
                    visited.add(neighbor)
                    """ 
                    node2parents[n].append(root)
                    node2num_paths[n]=len(node2parents[n])
        if neighbor in graph.neighbors(root) and (root not in node2parents[n]): 
                    if n!=root and (neighbor not in graph.neighbors(root)):
            """
                    node2parents[neighbor].append(parent)
                    node2num_paths[neighbor] = 1
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    for key,value in sorted_data:
        if(node2num_paths[key]>1):
            n=1/node2num_paths[key]
            for element in node2parents[key]:
                nodes[element]=nodes[element]+n
                nodes[key]=n  
        elif(node2num_paths[key]==1):
            for parent in node2parents[key]:
              nodes[parent]= nodes[parent]+nodes[key]
    for key,value in nodes.items(): 
        for parent in node2parents[key]:
            t.append(key)
            t.append(parent)
            t = sorted(t)
            ans[(t[0],t[1])]=nodes[key]
            t.clear()
    return ans
    pass
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in graph.nodes():
        ...
        ans1 = bottom_up(node, node2distances, node2num_paths, node2parents)
...
def partition_girvan_newman(graph, max_depth):
    ...
    ###TODO
    i=0;
    result=approximate_betweenness(graph,max_depth)
    graphCopy=graph.copy()
    result=sorted(result.items(),key=lambda tup:(-tup[1],tup[0]))
    components =[x for x in nx.connected_component_subgraphs(graphCopy)]
    while len(components)==1:
        graphCopy.remove_edge(*(result[i][0]))
        i+=1;
        components =[x for x in nx.connected_component_subgraphs(graphCopy)]
    return components
    pass
...
def get_subgraph(graph, min_degree):
    ...
    ###TODO
    sub=nx.Graph()
    degreegraph = graph.degree()
    for i in degreegraph:
        if degreegraph[i]>=min_degree:
            sub.add_node(i)
...
def norm_cut(S, T, graph):
    ...
    """
    ###TODO
    normcut= ((cut(S, T, graph)/float(volume(S, graph))) + (cut(S, T, graph)/float(volume(T, graph))))
    return normcut
    pass
...
def score_max_depths(graph, max_depths):
    ...
    ans=[]
    for d in max_depths:
        component=partition_girvan_newman(graph,d)
        component1=list(set(component[0].nodes()))
        component2=list(set(component[1].nodes()))
        l=norm_cut(component1,component2,graph)
        t=tuple((d,l))
        ans.append(t)
    return ans
    pass
...
def path_score(graph, root, k, beta):
    ...
    sortedList=[]
    for n in graph.nodes():
        if not graph.has_edge(root, n) and n != root:
            node2distances, node2num_paths, node2parents = bfs(graph,root,math.inf)
            s = math.pow(beta,node2distances[n]) * node2num_paths[n]
