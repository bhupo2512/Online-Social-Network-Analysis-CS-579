# -*- coding: utf-8 -*-
"""
Created on Sun Aug 28 15:45:44 2016
...
def read_screen_names(filename):
    ...
    with open(filename) as f:
        content = f.read().splitlines() #read candidates from file
    f.close
    return content
...
def add_all_friends(twitter, users):
    ...
    ###TODO
    for user in users:
        user['friends']=get_friends(twitter,user['screen_name']) #adding all friends to dictionary
...
def print_num_friends(users):
    ...
    ###TODO
    for user in users:
        print(user['screen_name']+"  "+str(user['friends_count'])) #number of friends per candidate
    pass
...
def count_friends(users):
    ...
    """
    ###TODO
    #counting friends per candidate
    counter=Counter()
    for user in users:
    ...
    for user1 in users:
        for user2 in users:
            if(user1['screen_name']!=user2['screen_name']):
                t=(user1['screen_name'],user2['screen_name'],len(set(user1['friends']).intersection(user2['friends'])))
                list.append(t)
    uniq=[]
    for i in list:                           # O(n), n being the size of l
        if not (i in uniq or tuple([i[1], i[0], i[2]]) in uniq): # O(n)
             uniq.append(i)
    uniq.sort(key=lambda tup: (-tup[2],tup[0],tup[1]))
    return uniq
    pass
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    for user1 in users:
        for user2 in users:
            if(user1['screen_name']=="realDonaldTrump" and user2['screen_name']=="HillaryClinton"):
                tuple=(user1['screen_name'],user2['screen_name'],set(user1['friends']).intersection(user2['friends']))
                list.append(tuple)
    userId = list[0][2]
    request=robust_request(twitter,'users/lookup', {'user_id':userId})
    #request = twitter.request('users/lookup', {'user_id':userId}) 
...
def draw_network(graph, users, filename):
    ...
    listScreen=[]
    for user in users:
        listScreen.append(user['screen_name'])
    pos=nx.spring_layout(graph)
    for node in graph.nodes():
        if node in listScreen:
            labelsNode[node] = node   
    plt.figure(figsize=(20,20))
    nx.draw_networkx(graph,pos=pos, with_labels=False)
    nx.draw_networkx_labels(graph,pos,labels=labelsNode,font_size=16,font_color='r',alpha=.6, width=.2,node_size=700)
    plt.axis("off")
